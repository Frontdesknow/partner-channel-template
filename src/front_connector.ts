...
export interface InboundMessageRequest extends BaseMessageRequest {
  sender: FrontSender;
}
...
export class FrontConnector {
   ...
   * @returns
   */
  static async importInboundMessage(
    channelId: string,
   ...
   * @returns
   */
  static async importOutboundMessage(
    channelId: string,
  ...
  private static async makeChannelAPIRequest(
    channelId: string,
    path: string,
    payload: InboundMessageRequest | OutboundMessageRequest
  ) {
    // If the payload has any attachments then we must send the request as multipart instead of application/json
    const hasAttachments =
      payload.attachments && payload.attachments.length > 0;
    const options = {
      headers: this.buildHeaders(channelId, hasAttachments),
