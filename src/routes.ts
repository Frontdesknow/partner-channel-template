...
import { FrontConnector, InboundMessageRequest } from "./front_connector";
...
ChannelRouter.post("/", async (req: Request, res: Response) => {
  ...
  console.log(
    `Creating webhook with URL ${webhookUrl} for channel with ID ${req.body.payload.channel_id}`
  );
  /**
...
ChannelRouter.post("/front/:webhookId", async (req: Request, res: Response) => {
  if (
    !["message", "message_autoreply", "message_imported"].includes(
      req.body.type
    )
  ) {
    return res.send(400).json({
      type: "bad_request",
      message: "Unknown message type sent to channel",
    });
  }
...
ChannelRouter.delete(
  ...
  async (req: Request, res: Response) => {
    if (req.body.type !== "delete") {
      res.send(400).json({
        type: "bad_request",
        message: "Unknown delete type sent to channel",
      });
    }
...
ChannelRouter.post("/oauth/token", async (req: Request, res: Response) => {
  ...
  return res
    .status(200)
    .json({ access_token: randomString(32), refresh_token: randomString(32) });
});

ChannelRouter.get("/importMessages", async (req: Request, res: Response) => {
  console.log("Importing messages from Front");

  const payload: InboundMessageRequest = {
    sender: {
      handle: "ex do nisi",
...
function verifyFrontRequest(req: Request, res: Response, next: NextFunction) {
  ...
  if (hmac !== req.headers["x-front-signature"]) {
    return res
      .send(400)
      .json({ type: "bad_request", message: "Signature not verified" });
  }
